...
import numpy as np
import pandas as pd
from settings import learning_rate, sequence_size, batch_size, hidden_layers_separate_models, test_data_filename
...
class FedAvg:
    ...
    def __init__(self, _model_name, _test_dataset_filename):
        ...
        FedAvg.y_test = dataset['me_power'].values.reshape(-1, 1)
        ...
        self.test_X, self.test_y = to_sequences(self.X_test_scaled, FedAvg.y_test[:, :], sequence_size)
...
def run_score_fed_avg():
    model_name = 'Fed_Avg_LR_{}_SS_{}_BS_{}_HL_{}'.format(learning_rate, sequence_size, batch_size,
                                                          hidden_layers_separate_models)
    test_dataset = FedAvg(model_name, test_data_filename)

    print('END Score FedAvg')
    return test_dataset.model_run()


if __name__ == '__main__':
    results = run_score_fed_avg()
    print('END Score FedAvg')
